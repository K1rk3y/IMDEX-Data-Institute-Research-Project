...
try:
    from mamba_ssm.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj, mamba_inner_fn_no_out_proj_sl
except ImportError:
...
class MambaSL(nn.Module):
    ...
    def forward(self, hidden_states, valid_positions=None, inference_params=None, T=1):
        """
        hidden_states: (B, L, D)
        valid_positions: (B, L) boolean mask
        Returns: same shape as hidden_states
        """
        batch, seqlen, dim = hidden_states.shape
        conv_state, ssm_state = None, None
        if inference_params is not None:
            ...
            if inference_params.seqlen_offset > 0:
                ...
                out, _, _ = self.step(hidden_states, conv_state, ssm_state)
                return out
        # Perform input projection and rearrange
        xz = rearrange(
            self.in_proj.weight @ rearrange(hidden_states, "b l d -> d (b l)"),
            "d (b l) -> b d l",
            l=seqlen,
        ...
        if self.use_fast_path and inference_params is None:
            if self.bimamba:
                ...
                out = mamba_inner_fn_no_out_proj_sl(
                    ...
                    delta_bias=self.dt_proj.bias.float(),
                    delta_softplus=True,
                    valid_positions=valid_positions,  # Pass valid_positions
                )
                # Forward pass for the backward direction
                out_b = mamba_inner_fn_no_out_proj_sl(
                    xz.flip(-1),  # Reverse the sequence
                    self.conv1d_b.weight,
                    self.conv1d_b.bias,
                    self.x_proj_b.weight,
                    self.dt_proj_b.weight,
                    ...
                    delta_bias=self.dt_proj_b.bias.float(),
                    delta_softplus=True,
                    valid_positions=valid_positions.flip(-1) if valid_positions is not None else None,  # Reverse the mask
                )
